...
@app.cell
def _(mo):
    latitude = mo.ui.number(
        start=-90, stop=90, value=55.79, step=0.01, label="Latitude [°N]"
    )
    longitude = mo.ui.number(
        start=-180, stop=180, value=12.53, step=0.01, label="Longitude [°E]"
    )
    altitude = mo.ui.number(
        start=-10, stop=8000, value=0, step=0.1, label="Altitude [m]"
    )
    ground_offset = mo.ui.number(
        start=-100, stop=200, value=0, step=0.1, label="Ground offset [m]"
    )
